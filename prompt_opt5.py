...
def extract_topics_and_generate_template(scenes, system_prompt1_file, system_prompt2_file):
    ...
    for scene_key, scene_data in scenes.items():
        ...
        for topic_data in possible_topics:
            ...
            # 获取AI的第一轮回应
            response_ai_first_round = call_api(messages_for_ai_first_round)
            ai_response_first_round = response_ai_first_round.choices[0].message.content.strip()
            
            # 更新第一轮对话的AI回应
            first_round["ai"] = ai_response_first_round
            
            # 第二轮对话：用户根据AI的回应进行回答
            messages_for_user_second_round = messages_for_user_first_round.copy()
            messages_for_user_second_round.append({"role": "assistant", "content": ai_response_first_round})
            messages_for_user_second_round.append({"role": "user", "content": user_input_for_topic})
            
            # 获取AI的第二轮回应
            response_ai_second_round = call_api(messages_for_ai_first_round + [{"role": "assistant", "content": ai_response_first_round}])
            ...
            for topic_data_third_round in possible_topics:
                ...
                # 创建API调用消息（第三轮）
                messages_for_user_third_round = messages_for_user_second_round.copy()
                messages_for_user_third_round.append({"role": "assistant", "content": ai_response_second_round})
                messages_for_user_third_round.append({"role": "user", "content": user_input_for_topic_third_round})
                
                # 获取AI的第三轮回应
                response_ai_third_round = call_api(messages_for_ai_first_round + [
                    {"role": "assistant", "content": ai_response_first_round},
                    {"role": "assistant", "content": ai_response_second_round}
                ])
                ai_response_third_round = response_ai_third_round.choices[0].message.content.strip()
                
                # 第三轮对话
                third_round = {
                    "user": user_input_for_topic_third_round,
...
if __name__ == '__main__':
    ...
    # 将生成的JSON保存到文件
    output_file_path = "/mnt/afs/chenyun/zhangjing/prompt_opt/output_template4.json"  # 输出文件路径
